---
description: Instala los contratos de selección de next-task (wrapper de compatibilidad que delega en devflow init next-task)
agent: general
subtask: true
---
...
```bash
node <RUTA_REPO>/bin/devflow.mjs init next-task
```
...
- El comando canónico es `/devflow-init next-task`.
